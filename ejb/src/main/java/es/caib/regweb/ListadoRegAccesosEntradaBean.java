...
/**
 * Bean que permet fer consultes sobre el registre d'accessos al registre d'entrades 
 * @author  Sebastià Matas, basat en codi de FJMARTINEZ
 * @version 1.0
 */

public class ListadoRegAccesosEntradaBean implements SessionBean {
	
	private static final Category log = Category.getInstance(ListadoRegAccesosEntradaBean.class.getName());
	...
	public ListadoRegAccesosEntradaBean() {
	}
...
//			Del tipus: 00:00:01:zzz
	...
	public Vector recuperar(String usuario, int sizePagina, int pagina) throws java.rmi.RemoteException {
		...
		try {
			...
			String sentenciaSql="SELECT * FROM BZENLPD LEFT JOIN BAGECOM ON FAACAGCO=FZTCAGCO " +
			"WHERE FZTCAGCO>=? AND FZTCAGCO<=? " +
			(!fechaDesde.equals("") ? " AND FZTDATAC>=? AND FZTDATAC<=? " : "" ) +
			(!numRegistre.equals("") ? " AND FZTNUMEN=?" : "" )+
			(!anyRegistre.equals("") ? " AND FZTANOEN=?" : "" )+
			( !numRegistre.equals("") || !anyRegistre.equals("") ? 
					" ORDER BY FZTANOEN, FZTNUMEN, FZTDATAC, FZTHORAC" :
					" ORDER BY FZTDATAC, FZTHORAC, FZTANOEN, FZTNUMEN ");			
...
//				 Point to the last row in resultset.
			...
			while (rs.next()) {
				...
				registro.setTipusAcces(rs.getString("FZTTIPAC"));
				registro.setUsuCanvi(rs.getString("FZTCUSU"));			
				String fechaES=String.valueOf(rs.getInt("FZTDATAC"));
				try {
				...
				/**  TODO: PARSEJAR HORA */   
				registro.setHoraCanvi( parsejarHora(String.valueOf(rs.getInt("FZTHORAC"))) );
				registro.setNombreRegistre(String.valueOf(rs.getInt("FZTNUMEN")));
				registro.setAnyRegistre(String.valueOf(rs.getInt("FZTANOEN")));
				registro.setOficinaRegistre(String.valueOf(rs.getInt("FZTCAGCO")));
				...
				psHist.setString(1,String.valueOf(rs.getInt("FZTCAGCO")));
