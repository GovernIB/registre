...
public class RegistroModificadoEntradaBean implements SessionBean {
	...
	public void leer() {
		...
		try {
			...
			String sentenciaSql="SELECT * FROM BZMODIF WHERE FZJCENSA='E' AND FZJCAGCO=? AND FZJNUMEN=? AND FZJANOEN=? AND" +
			...
			if (rs.next()) {
	            ...
	            logLopdBZMODIF("SELECT", sessioEjb.getCallerPrincipal().getName().toUpperCase() 
						, fzafsis, horamili, 'E', numeroRegistro, anoEntrada, oficina, Integer.parseInt(fechaModificacion), Integer.parseInt(horaModificacion) );
	...
	public boolean visar() throws SQLException, ClassNotFoundException, Exception {
		...
		try {
			...
			/* SMR: Alerta!, aquest update potser sigui de més d'un registre!, tenir-ho en compte per la LOPD. */ 
			...
			if (registrosAfectados>0 && (hayVisadoExtracto || hayVisadoRemitente) ) {
				...
				if (generado) { // Aztualizamos el BZENTRA
                ...
                try {
                    ...
                    Object [] params = {
                        "V", new Integer(anoEntrada), new Integer(numeroRegistro), new Integer(oficina), new Integer(fechaDocumento), rem, 
                    ...
                    //t.entrada("V", anoEntrada, numeroRegistro, oficina, fechaDocumento, rem,
	...
	private boolean actualizarBZENTRA(Connection conn) throws SQLException, ClassNotFoundException, Exception {
		...
		String actualizaBZENTRA="UPDATE BZENTRA SET FZAFACTU=? " +
		((hayVisadoExtracto && idiomaExtracto.equals("1")) ? ", FZACONEN=?" : "") +
		((hayVisadoExtracto && !idiomaExtracto.equals("1")) ? ", FZACONE2=?" : "") +
		//((hayVisadoExtracto && hayVisadoRemitente) ? ", " : "") +
		((hayVisadoRemitente) ? ", FZAREMIT=?, FZACENTI=?, FZANENTI=?" : "" ) +
		" WHERE FZANUMEN=? AND FZACAGCO=? AND FZAANOEN=?";
		try {
            ...
            int horamili=Integer.parseInt(hhmmss.format(fechaSystem)+Stringsss);  
            int fzafsis=Integer.parseInt(aaaammdd.format(fechaSystem));
            logLopdBZENTRA("UPDATE", usuarioModificacion
    				, fzafsis, horamili, numeroRegistro, anoEntrada, oficina);
		} catch (Exception e){
			System.out.println("Error: "+e.getMessage());
			generado=false;
			e.printStackTrace();
	...
	public List recuperarRegistros(String oficina, String usuario) {
		...
		if (oficina.equals("00")) {
			...
			"IN (SELECT FZHCAGCO FROM BZAUTOR WHERE FZHCUSU=? AND FZHCAUT=?) AND FZJCENSA='E' ORDER BY " +
			"FZJCAGCO, FZJANOEN, FZJNUMEN, FZJFMODI, FZJHMODI";
		} else {
			sentenciaSql="SELECT * FROM BZMODIF WHERE FZJCAGCO=? AND FZJFVISA=0 AND (FZJIEXTR=' ' OR FZJIREMI=' ' OR FZJIEXTR='' OR FZJIREMI='') AND FZJCENSA='E' ORDER BY " +
			"FZJCAGCO, FZJANOEN, FZJNUMEN, FZJFMODI, FZJHMODI";
		}
		try {
			conn=ToolsBD.getConn();
			...
			while (rs.next()) {
				...
				if ( (rs.getString("FZJIREMI").equals(" ") && (!rs.getString("FZJCENTI").trim().equals("") || !rs.getString("FZJREMIT").trim().equals("")) )
						|| (rs.getString("FZJIEXTR").equals(" ") && !rs.getString("FZJCONEN").trim().equals("")) ) {
					registros.add(reg);
				}
			}
		} catch (Exception e) {
	...
	public boolean generarModificacion(Connection conn) {
		...
		try {
			...
			logLopdBZMODIF("INSERT", usuarioModificacion
					, Integer.parseInt(aaaammdd.format(fechaSystem)), horamili, 'E', numeroRegistro, anoEntrada, oficina, Integer.parseInt(aaaammdd.format(fechaSystem)),Integer.parseInt(hhmmss.format(fechaSystem)+ss) );
    ...
    private void logLopdBZENTRA(String tipusAcces, String usuari, int data, int hora, int nombreRegistre, int any, int oficina ) {
		...
		//System.out.println("RegistroModificadoEntradaBean: Desada informació dins BZENLPD: "+tipusAcces+" "+usuari+" "+data+" "+hora+" "+nombreRegistre+" "+any+" "+oficina);
     }
}