...
update RWE_REGISTRO_DETALLE set presencial=0 where id in(select re.registro_detalle from RWE_REGISTRO_ENTRADA re,rwe_usuario usu, RWE_USUARIO_ENTIDAD ue where usu.tipousuario=2 and usu.id=ue.USUARIO and ue.id=re.usuario);
update RWE_REGISTRO_DETALLE set presencial=0 where id in(select rs.registro_detalle from RWE_REGISTRO_SALIDA rs,rwe_usuario usu, RWE_USUARIO_ENTIDAD ue where usu.tipousuario=2 and usu.id=ue.USUARIO and ue.id=rs.usuario);
