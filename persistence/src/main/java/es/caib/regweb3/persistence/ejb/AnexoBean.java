...
public class AnexoBean extends BaseEjbJPA<Anexo, Long> implements AnexoLocal {
    ...
    public AnexoFull crearJustificanteAnexo(AnexoFull anexoFull, UsuarioEntidad usuarioEntidad,
        ...
        try {
            ...
            if (!anexo.isJustificante() && validarAnexo && !RegwebConstantes.TIPO_DOCUMENTO_FICHERO_TECNICO_SICRES.equals(anexo.getTipoDocumento().toString())) { //Solo validamos si no es justificante
            ...
            try {
                integracionEjb.addIntegracionError(RegwebConstantes.INTEGRACION_CUSTODIA, descripcion, peticion.toString(), i18n,null, System.currentTimeMillis() - tiempo, usuarioEntidad.getEntidad().getId(), numRegFormat);
            } catch (Exception e) {
                e.printStackTrace();
            }
            ...
            try {
                integracionEjb.addIntegracionError(RegwebConstantes.INTEGRACION_CUSTODIA, descripcion, peticion.toString(), e, null,System.currentTimeMillis() - tiempo, usuarioEntidad.getEntidad().getId(), numRegFormat);
            } catch (Exception ex) {
                e.printStackTrace();
            }
