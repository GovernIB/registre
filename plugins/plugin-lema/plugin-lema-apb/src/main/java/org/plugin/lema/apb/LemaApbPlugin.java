package org.plugin.lema.apb;

import java.net.URL;
import java.util.HashMap;
import java.util.Map;
import java.util.Properties;

import javax.xml.namespace.QName;
import javax.xml.ws.soap.SOAPFaultException;

import org.apache.cxf.configuration.jsse.TLSClientParameters;
import org.apache.cxf.endpoint.Client;
import org.apache.cxf.frontend.ClientProxy;
import org.apache.cxf.interceptor.LoggingInInterceptor;
import org.apache.cxf.interceptor.LoggingOutInterceptor;
import org.apache.cxf.message.Message;
import org.apache.cxf.transport.http.HTTPConduit;
import org.apache.cxf.transports.http.configuration.HTTPClientPolicy;
import org.apache.cxf.ws.security.wss4j.WSS4JOutInterceptor;
import org.apache.ws.security.handler.WSHandlerConstants;
import org.fundaciobit.pluginsib.core.utils.AbstractPluginProperties;
import org.plugin.lema.apb.helper.ConversionPluginHelper;
import org.plugin.lema.api.ConsultaAcuseReciboRequest;
import org.plugin.lema.api.ConsultaAcuseReciboResponse;
import org.plugin.lema.api.ConsultaAnexoRequest;
import org.plugin.lema.api.ConsultaAnexoResponse;
import org.plugin.lema.api.ConsultaRealizadaRequest;
import org.plugin.lema.api.ConsultaRealizadaResponse;
import org.plugin.lema.api.ILemaPlugin;
import org.plugin.lema.api.LemaPluginException;
import org.plugin.lema.api.LocalizaRealizadaResponse;
import org.plugin.lema.api.LocalizaRealizadasRequest;
import org.plugin.lema.api.LocalizaRequest;
import org.plugin.lema.api.LocalizaResponse;
import org.plugin.lema.api.PeticionAccesoRequest;
import org.plugin.lema.api.PeticionAccesoResponse;
import org.plugin.lema.api.realizadas.ws.ConsultaRealizadas;
import org.plugin.lema.api.realizadas.ws.Contenido;
import org.plugin.lema.api.realizadas.ws.ContenidoMtomInfo;
import org.plugin.lema.api.realizadas.ws.GDRealizadasWsPortType;
import org.plugin.lema.api.realizadas.ws.GDRealizadasWsService;
import org.plugin.lema.api.realizadas.ws.HashDocumento;
import org.plugin.lema.api.realizadas.ws.LocalizaRealizadas;
import org.plugin.lema.api.realizadas.ws.RespuestaConsultaRealizadas;
import org.plugin.lema.api.realizadas.ws.RespuestaLocalizaRealizadas;
import org.plugin.lema.api.ws.ConsultaAcusePdf;
import org.plugin.lema.api.ws.ConsultaAnexos;
import org.plugin.lema.api.ws.DEHuWsPortType;
import org.plugin.lema.api.ws.DEHuWsService;
import org.plugin.lema.api.ws.Localiza;
import org.plugin.lema.api.ws.PeticionAcceso;
import org.plugin.lema.api.ws.RespuestaConsultaAcusePdf;
import org.plugin.lema.api.ws.RespuestaConsultaAnexos;
import org.plugin.lema.api.ws.RespuestaLocaliza;
import org.plugin.lema.api.ws.RespuestaPeticionAcceso;

/**
 * Created by Limit Tecnologies S.L.
 * 
 * @author Jamal
 */
public class LemaApbPlugin extends AbstractPluginProperties implements ILemaPlugin {

	private static final String COMPARECER = "1";
	private static final QName SERVICE_NAME = new QName(
			"https://administracionelectronica.gob.es/notifica/ws/lema/1.0/", "DEHuWsService");
	private static final QName SERVICE_REALIZADAS_NAME = new QName(
			"https://administracionelectronica.gob.es/notifica/ws/lema/1.0/", "GDRealizadasWsService");

	private static final String basePluginLemaApb = LEMA_BASE_PROPERTY + "apb.";
	private static final String PROPERTY_LOCALIZA_WSDL = basePluginLemaApb + "service.localiza.wsdl";
	private static final String PROPERTY_LOCALIZA_REALIZADAS_WSDL = basePluginLemaApb
			+ "service.localiza.realizadas.wsdl";

	private static final String KEYSTORE_FILE = basePluginLemaApb + "keystore.file";
	private static final String KEYSTORE_TYPE = basePluginLemaApb + "keystore.type";
	private static final String KEYSTORE_ALIAS = basePluginLemaApb + "keystore.alias";
	private static final String KEYSTORE_PASS = basePluginLemaApb + "keystore.pass";

	private static final String TITULAR_NIF = basePluginLemaApb + "titular.nif";
	private static final String RECEPTOR_NIF = basePluginLemaApb + "receptor.nif";
	
	private static final String TITULAR_NIF_TEST = "Q0767004E";
	
	public LemaApbPlugin() {
		super();
	}

	public LemaApbPlugin(String propertyKeyBase, Properties properties) {
		super(propertyKeyBase, properties);
	}

	public LemaApbPlugin(String propertyKeyBase) {
		super(propertyKeyBase);
	}

	@Override
	public LocalizaResponse localiza(LocalizaRequest request) throws LemaPluginException {
		RespuestaLocaliza respuesta = null;

		try {
			request.setNifTitular((String) getPropertyAsunto(TITULAR_NIF));
			Localiza localiza = new ConversionPluginHelper().convertir(
					request, 
					Localiza.class);
			respuesta = deHuWsPort().localiza(localiza);

			System.out.println("localiza._localiza_codigoRespuesta=" + respuesta.getCodigoRespuesta());
			System.out.println("localiza._localiza_descripcionRespuesta=" + respuesta.getDescripcionRespuesta());
			System.out.println("localiza._localiza_nifPeticion=" + respuesta.getNifPeticion());
			System.out.println("localiza._localiza_envios=" + respuesta.getEnvios());

		} catch (SOAPFaultException ex) {
			throw new LemaPluginException("[LEMA] Ha habido un problema localizando los envíos pendientes. " + ex.getMessage(), ex.getCause());
		} catch (Exception ex) {
			throw new LemaPluginException("[LEMA] Ha habido un problema localizando los envíos pendientes. " + ex.getMessage(), ex.getCause());
		}
		return new ConversionPluginHelper().convertir(respuesta, LocalizaResponse.class);

	}

//	@Override
//	public ConsultaRealizadaResponse consultaRealizadas(ConsultaRealizadaRequest request) {
//		RespuestaConsultaRealizadas respuesta = null;
//		try {
//			ConsultaRealizadas consultaRealizadas = new ConversionPluginHelper().convertir(
//					request, 
//					ConsultaRealizadas.class);
////			respuesta = GDWsRealizadasPort().consultaRealizadas(consultaRealizadas);
//			respuesta = new RespuestaConsultaRealizadas();
//			respuesta.setCodigoOrigen("2");
//			respuesta.setCodigoRespuesta("200");
//			respuesta.setDescripcionRespuesta("Resultado de la consulta: ok");
//			org.plugin.lema.api.realizadas.ws.DetalleDocumento detalle = new org.plugin.lema.api.realizadas.ws.DetalleDocumento();
//				ContenidoMtomInfo contenido = new ContenidoMtomInfo();
//				Contenido contenidoConsulta = new Contenido();
//				contenidoConsulta.setContentType("application/pdf");
//				contenidoConsulta.getContent().add("");
//				contenido.setContenido(contenidoConsulta);
//				contenido.setTipoMIME("application/pdf");
//				detalle.setContenido(contenido);
//				detalle.setCsvResguardo("DEHU-5a074fb3377ec04120dd04249c67733d");
//				HashDocumento hash = new HashDocumento();
//				hash.setHash("M2RmNzlkMzRhYmJjYTk5MzA4ZTc5Y2I5NDQ2MWMxODkzNTgyNjA0ZDY4MzI5YTQxZmQ0YmVjMTg4NWU2YWRiNA==");
//				hash.setAlgoritmoHash("sha256");
//				detalle.setHashDocumento(hash);
//				detalle.setMimeType("application/pdf");
//				detalle.setNombre("PfedNKu8qZMI55y5RGHBiTWCYE1oMppB/UvsGIXmrbQ=.pdf");
//			respuesta.setDocumento(detalle);
//			respuesta.setFechaUltimoEstado(null);
//			respuesta.setIdentificador("7196331672b9054d0c2f");
//			respuesta.setPostal(false);
//			System.out.println("localiza._localiza_codigoRespuesta=" + respuesta.getCodigoRespuesta());
//			System.out.println("localiza._localiza_descripcionRespuesta=" + respuesta.getDescripcionRespuesta());
//
//		} catch (SOAPFaultException ex) {
//			throw new LemaPluginException("[LEMA] Ha habido un problema localizando los envíos realizados. " + ex.getMessage(), ex.getCause());
//		} catch (Exception ex) {
//			throw new LemaPluginException("[LEMA] Ha habido un problema localizando los envíos realizados. " + ex.getMessage(), ex.getCause());
//		}
//		
//		return new ConversionPluginHelper().convertir(
//				respuesta, 
//				ConsultaRealizadaResponse.class);
//
//	}
	
	@Override
	public PeticionAccesoResponse peticionAcceso(PeticionAccesoRequest request) throws LemaPluginException {
		RespuestaPeticionAcceso respuesta = null;
		
		try {
			request.setNifReceptor((String) getPropertyAsunto(RECEPTOR_NIF));
			PeticionAcceso peticionAcceso = new ConversionPluginHelper().convertir(
					request, 
					PeticionAcceso.class);
			
			respuesta = deHuWsPort().peticionAcceso(peticionAcceso);

//			respuesta.setCodigoRespuesta("200");
//			respuesta.setDescripcionRespuesta("Correcto");
//			DetalleDocumento detalle = new DetalleDocumento();
//			detalle.setEnlaceDocumento("www.google.es");
//			
//			respuesta.setDocumento(detalle);
			
			System.out.println("peticionAcceso._peticionAcceso_codigoRespuesta=" + respuesta.getCodigoRespuesta());
			System.out.println("peticionAcceso._peticionAcceso_descripcionRespuesta=" + respuesta.getDescripcionRespuesta());
			System.out.println("peticionAcceso._peticionAcceso_identificador1=" + respuesta.getIdentificador());
			System.out.println("peticionAcceso._peticionAcceso_codigoOrigen1=" + respuesta.getCodigoOrigen());
			System.out.println("peticionAcceso._peticionAcceso_fechaEvento=" + respuesta.getFechaEvento());
			System.out.println("peticionAcceso._peticionAcceso_documento=" + respuesta.getDocumento());
			System.out.println("peticionAcceso._peticionAcceso_opcionesRespuestaPeticionAcceso=" + respuesta.getOpcionesRespuestaPeticionAcceso());
			
		} catch (SOAPFaultException ex) {
			ex.printStackTrace();
			throw new LemaPluginException("[LEMA] Ha habido un problema accediento al envío pendiente [identificador=" +request.getIdentificador() + "]");
		} catch (Exception ex) {
			ex.printStackTrace();
			throw new LemaPluginException("[LEMA] Ha habido un problema accediento al envío pendiente [identificador=" +request.getIdentificador() + "]");
		}
		
		return new ConversionPluginHelper().convertir(respuesta, PeticionAccesoResponse.class);
	}

//	@Override
//	public LocalizaRealizadaResponse localizaRealizadas(LocalizaRealizadasRequest request) {
//		RespuestaLocalizaRealizadas respuesta = null;
//		try {
//			request.setNifTitular((String) getPropertyAsunto(TITULAR_NIF));
//			LocalizaRealizadas localizaRealizadas = new ConversionPluginHelper().convertir(
//					request, 
//					LocalizaRealizadas.class);
//			respuesta = GDWsRealizadasPort().localizaRealizadas(localizaRealizadas);
//
//			System.out.println("localiza._localiza_codigoRespuesta=" + respuesta.getCodigoRespuesta());
//			System.out.println("localiza._localiza_descripcionRespuesta=" + respuesta.getDescripcionRespuesta());
//			System.out.println("localiza._localiza_nifPeticion=" + respuesta.getNifPeticion());
//			System.out.println("localiza._localiza_envios=" + respuesta.getEnvios());
//
//		} catch (SOAPFaultException ex) {
//			throw new LemaPluginException("[LEMA] Ha habido un problema localizando los envíos realizados. " + ex.getMessage(), ex.getCause());
//		} catch (Exception ex) {
//			throw new LemaPluginException("[LEMA] Ha habido un problema localizando los envíos realizados. " + ex.getMessage(), ex.getCause());
//		}
//		
//		return new ConversionPluginHelper().convertir(respuesta, LocalizaRealizadaResponse.class);
//
//	}

	@Override
	public ConsultaAcuseReciboResponse consultaAcuseRecibo(ConsultaAcuseReciboRequest request) {
		RespuestaConsultaAcusePdf respuesta = null;
		try {
			request.setNifReceptor((String) getPropertyAsunto(RECEPTOR_NIF));
			ConsultaAcusePdf consultaAcuseRecibo = new ConversionPluginHelper().convertir(
					request, 
					ConsultaAcusePdf.class);
			respuesta = deHuWsPort().consultaAcusePdf(consultaAcuseRecibo);

			System.out.println("consulta._acuse_codigoRespuesta=" + respuesta.getCodigoRespuesta());
			System.out.println("consulta._acuse_descripcionRespuesta=" + respuesta.getDescripcionRespuesta());
			System.out.println("consulta._acuse_nombre=" + respuesta.getAcusePdf().getContenido().getHref().getInputStream());

		} catch (SOAPFaultException ex) {
			ex.printStackTrace();
			throw new LemaPluginException("[LEMA] Ha habido un problema localizando el acuse de recibo del envío [identificador=" +request.getIdentificador() + "]");
		} catch (Exception ex) {
			ex.printStackTrace();
			throw new LemaPluginException("[LEMA] Ha habido un problema localizando el acuse de recibo del envío [identificador=" +request.getIdentificador() + "]");
		}
		
		return new ConversionPluginHelper().convertir(respuesta, ConsultaAcuseReciboResponse.class);

	}
	
	@Override
	public ConsultaAnexoResponse consultaAnexo(ConsultaAnexoRequest request) {
		RespuestaConsultaAnexos respuesta = null;
		try {
			request.setNifReceptor((String) getPropertyAsunto(RECEPTOR_NIF));
			ConsultaAnexos consultaAnexos = new ConversionPluginHelper().convertir(
					request, 
					ConsultaAnexos.class);
			respuesta = deHuWsPort().consultaAnexos(consultaAnexos);

			System.out.println("consulta._anexo_codigoRespuesta=" + respuesta.getCodigoRespuesta());
			System.out.println("consulta._anexo_descripcionRespuesta=" + respuesta.getDescripcionRespuesta());
			System.out.println("consulta._anexo_nombre=" + respuesta.getDocumento().getNombre());

		} catch (SOAPFaultException ex) {
			ex.printStackTrace();
			throw new LemaPluginException("[LEMA] Ha habido un problema localizando el anexo [referencia=" + request.getReferenciaAnexo() + "] del envío [identificador=" + request.getIdentificador() + "]");
		} catch (Exception ex) {
			ex.printStackTrace();
			throw new LemaPluginException("[LEMA] Ha habido un problema localizando el anexo [referencia=" + request.getReferenciaAnexo() + "] del envío [identificador=" + request.getIdentificador() + "]");
		}
		
		return new ConversionPluginHelper().convertir(respuesta, ConsultaAnexoResponse.class);

	}
	
	@Override
	public String consultaPropietat(String propietat) {
		return (String) getProperty(basePluginLemaApb + propietat);
	}
	
	public DEHuWsPortType deHuWsPort() throws Exception {
		// Ubicación del WSDL
		URL wsdlURL = new URL((String) getPropertyAsunto(PROPERTY_LOCALIZA_WSDL));
//		URL wsdlURL = new URL("https://se-dehuws.redsara.es/wsdl/GD_Dehu/v2/Gd-Dehu-Ws_se.wsdl");
		
		// Crear la instancia del servicio SOAP usando el WSDL y el QName
		DEHuWsService ss = new DEHuWsService(wsdlURL, SERVICE_NAME);
		DEHuWsPortType port = ss.getDEHuWsPort();

		String newEndpointAddress = null; //"https://se-gd-dehuws.redsara.es/ws/v2/lema";
		
		return authenticate(port, newEndpointAddress);
	}

	public GDRealizadasWsPortType GDWsRealizadasPort() throws Exception {
		// Ubicación del WSDL
		URL wsdlURL = new URL((String) getPropertyAsunto(PROPERTY_LOCALIZA_REALIZADAS_WSDL));
//		URL wsdlURL = new URL("https://se-gd-dehuws.redsara.es/ws/v1/realizadas?wsdl");

		// Crear la instancia del servicio SOAP usando el WSDL y el QName
		GDRealizadasWsService ss = new GDRealizadasWsService(wsdlURL, SERVICE_REALIZADAS_NAME);
		GDRealizadasWsPortType port = ss.getGDRealizadasWsPort();

		String newEndpointAddress = "https://se-gd-dehuws.redsara.es/ws/v1/realizadas";

		return authenticate(port, newEndpointAddress);
	}

	private <T> T authenticate(T port, String newEndpointAddress) {
		// Configurar el HTTPConduit para la autenticación SSL/TLS
	    Client client = ClientProxy.getClient(port);
	    if (newEndpointAddress != null)
		    client.getRequestContext().put(Message.ENDPOINT_ADDRESS, newEndpointAddress);
		HTTPConduit httpConduit = (HTTPConduit) client.getConduit();
		TLSClientParameters tlsClientParameters = new TLSClientParameters();

		Map<String, Object> outProps = new HashMap<>();
		outProps.put(WSHandlerConstants.ACTION, "Signature");
//		outProps.put(WSHandlerConstants.USER, getPropertyAsunto(KEYSTORE_ALIAS));
		outProps.put(WSHandlerConstants.USER, "auth");
		outProps.put(WSHandlerConstants.PW_CALLBACK_CLASS, KeystorePasswordCallback.class.getName());
		outProps.put(WSHandlerConstants.SIG_KEY_ID, "DirectReference");
		outProps.put(WSHandlerConstants.MUST_UNDERSTAND, "false");
		outProps.put(WSHandlerConstants.USE_SINGLE_CERTIFICATE, "true");
		outProps.put(WSHandlerConstants.SIG_ALGO, "http://www.w3.org/2000/09/xmldsig#rsa-sha1");
		outProps.put(WSHandlerConstants.SIG_DIGEST_ALGO, "http://www.w3.org/2000/09/xmldsig#sha1");
		outProps.put(WSHandlerConstants.SIGNATURE_PARTS, "{Content}{}Body");
		outProps.put(WSHandlerConstants.ENABLE_SIGNATURE_CONFIRMATION, "true");

		Properties merlinProperties = new Properties();
		merlinProperties.put("org.apache.ws.security.crypto.merlin.file", getPropertyAsunto(KEYSTORE_FILE));
		merlinProperties.put("org.apache.ws.security.crypto.provider", "org.apache.ws.security.components.crypto.Merlin");
		merlinProperties.put("org.apache.ws.security.crypto.merlin.keystore.type", getPropertyAsunto(KEYSTORE_TYPE));
		merlinProperties.put("org.apache.ws.security.crypto.merlin.keystore.password", getPropertyAsunto(KEYSTORE_PASS));
		merlinProperties.put("org.apache.ws.security.crypto.merlin.keystore.alias", getPropertyAsunto(KEYSTORE_ALIAS));

//		merlinProperties.put("org.apache.ws.security.crypto.merlin.file", "./doc/lema_pre.jks");
//		merlinProperties.put("org.apache.ws.security.crypto.provider", "org.apache.ws.security.components.crypto.Merlin");
//		merlinProperties.put("org.apache.ws.security.crypto.merlin.keystore.type", "JKS");
//		merlinProperties.put("org.apache.ws.security.crypto.merlin.keystore.password", "apb1234");
//		merlinProperties.put("org.apache.ws.security.crypto.merlin.keystore.alias", "auth");
		
		outProps.put("signingProperties", merlinProperties);
		outProps.put(WSHandlerConstants.SIG_PROP_REF_ID, "signingProperties");

		WSS4JOutInterceptor wssOut = new WSS4JOutInterceptor(outProps);
		client.getEndpoint().getOutInterceptors().add(wssOut);

		// Configurar para evitar la verificación de CN si es necesario (opcional)
		tlsClientParameters.setDisableCNCheck(true);

		client.getInInterceptors().add(new LoggingInInterceptor());
		client.getOutInterceptors().add(new LoggingOutInterceptor());

		httpConduit.setTlsClientParameters(tlsClientParameters);

		HTTPClientPolicy policy = new HTTPClientPolicy();
		policy.setConnectionTimeout(1000);
		policy.setReceiveTimeout(60000);
		policy.setAllowChunking(false);
		httpConduit.setClient(policy);

		return (T) port;
	}

	@SuppressWarnings("unchecked")
	private <T> T getPropertyAsunto(String atribut) {
		try {
			return (T) getPropertyRequired(atribut);
		} catch (Exception e) {
			throw new LemaPluginException("No s'ha definit la propietat '" + atribut + "'");
		}
	}

}
