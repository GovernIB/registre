...
session.setAttribute("listadoRegAcESEntrada",parametros);
%>

<% boolean ok=listado.validarBusqueda(parametros);
if (!ok){
%>
    <jsp:forward page="busquedaRegAccEntradasXFechas.jsp" />
<% } else { %>
...
<body>
		...
		<ul id="mollaPa">
			...
			<li><a href="busquedaRegAccEntradasXFechas.jsp"><fmt:message key='consulta_log_entrades'/></a></li>
			<li><fmt:message key='lopd.llistat_log_entrades'/></li>
		</ul>
		<!-- Fi Molla pa-->
		<p>&nbsp;</p>
<%
...
<p>&nbsp;</p>
<p>&nbsp;</p>
<center>[&nbsp;<a href="busquedaRegAccEntradasXFechas.jsp"><fmt:message key='consulta_log_entrades'/></a>&nbsp;]</center>
     <% } else { %>
...
<table border="0" align="center"><!--width="95%"--> 
    <tr>
        <td align="left" width="33%">
            ...
            if (pagina>1) {
            %>
            <a href="listadoRegAccEntrada.jsp?<%=cadenaEnlace%>&pagina=<%=pagina-1%>" title="Retrocedir" style="text-decoration:none;"><< 100 <fmt:message key='anteriors'/></a>
            <%
            }
            %>
        </td>
        <td align="center" width="33%">
            [&nbsp;<a href="<%=(request.getParameter("any")==null) ? "busquedaRegAccEntradasXFechas.jsp" : "busquedaRegAccEntradasXRegistro.jsp"%>"><fmt:message key='tornar_seleccionar'/></a>&nbsp;]
        </td>
        <td align="right" width="33%">
            <%
            if (registros.size()>sizePagina) {
            %>
            <a href="listadoRegAccEntrada.jsp?<%=cadenaEnlace%>&pagina=<%=pagina+1%>" title="Avançar" style="text-decoration:none;">100 <fmt:message key='seguents'/> >></a>
            <%
            }
            %>
        </td>
