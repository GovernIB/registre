...
<script>
		...
		function abreModelos() {
            	miDestinatarios=open("<c:url value='/admin/popup.do?accion=totsModelsOficis'/>","models","scrollbars,resizable,width=400,height=400");
...
<body>
		...
		<ul id="mollaPa">
			...
			<li><fmt:message key="gestio_models_oficis"/></li>
		</ul>
		<!-- Fi Molla pa-->
		<p>&nbsp;</p>
    ...
    <br/>
    		...
    		<p>&nbsp;</p>  
                	 ...
                	 <ul style="margin-right: 5px">
                    	...
                    	<li>	<form id="cercaModel" name="cercaModel" action="<c:url value='/admin/controller.do?accion=modelsOficis'/>" method="post">
...
<p>&nbsp;</p>
    ...
    <br/>
    <form name="gestionaModels" id="gestionaModels" action="<c:url value='/admin/controller.do?accion=altaModelOfici'/>" method="post"  enctype="multipart/form-data">	
    ...
    <table>
    <tr>
    	<th><fmt:message key='nom_model_ofici'/></th>
    ...
    <c:forEach var="item" begin='0' end='${descModelSize-1}' step='2'>
  	...
  	<c:set var="content" value="${descModel[item+1]}" />
  	  <tr>
  	    ...
  	    <td><c:out escapeXml="false" value='${content}'/></td>
			  <td><a href="<c:url value='/mostrafitxer?nom=${nom}'/>" target="_blank"><fmt:message key='veure'/></a></td>
	  ...
	  <c:otherwise>
	  <p><fmt:message key="model_ofici_no_existeix"><fmt:param value="${modelGestionar}"/></fmt:message></p>
