...
try {
    ...
    try {
      ...
      if (loginInfo != null) {
        String idioma = RegwebConstantes.CODIGO_BY_IDIOMA_ID.get(loginInfo.getUsuarioAutenticado().getIdioma());
...
<body>
 <div class="alert alert-error">
      ...
      <div>
        ...
        <% if (!loginError) { %>
        <a href="<c:url value="/index.jsp"/>" class="btn"><%=etiquetaBoto%></a>
        <% } %>
        ...
        <c:if test="${not empty stacktrace}">
            <div class="trace" style="display:none;"><b><%=detallError%></b></div>
            <c:forEach var="trace" items="${stacktrace}">
              <p class="trace" style="display:none;">${trace}</p>
            </c:forEach>
            <br/>
         </c:if>
       <% } %>
