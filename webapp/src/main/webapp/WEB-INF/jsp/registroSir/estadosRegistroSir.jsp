...
<c:choose>
    ...
    <c:when test="${registroSir.estado == 'REENVIADO'}">
        <p rel="popupAbajo" data-content="<c:out value="${registroSir.decodificacionTipoAnotacion}" escapeXml="true"/>" data-toggle="popover"><span class="label label-warning"><spring:message code="registroSir.estado.${registroSir.estado}" /></span></p>
    </c:when>
    <c:when test="${registroSir.estado == 'RECHAZADO'}">
        <p rel="popupAbajo" data-content="<c:out value="${registroSir.decodificacionTipoAnotacion}" escapeXml="true"/>" data-toggle="popover"><span class="label label-danger"><spring:message code="registroSir.estado.${registroSir.estado}" /></span></p>
    </c:when>
