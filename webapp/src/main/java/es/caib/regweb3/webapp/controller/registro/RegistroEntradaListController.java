...
//        registroEntradaBusqueda.setIdOrganismo(seleccionarOrganismoActivo(request, organismosConsultaEntrada));
    ...
    public String detalleRegistroEntrada(@PathVariable Long idRegistro, Model model, HttpServletRequest request) throws Exception, I18NException {
        ...
        if (remesas != null) {
        	...
        	registro.setIdentificadorInternRemesa(remesas.get(0).getIdentificadorIntern());
        }
        Entidad entidadActiva = getEntidadActiva(request);
        UsuarioEntidad usuarioEntidad = getUsuarioEntidadActivo(request);
        Oficina oficinaActiva = getOficinaActiva(request);
        ...
        String destinoExterno = registro.getDestinoExternoCodigo();
        ...
        if (registro.getEstado() != RegwebConstantes.REGISTRO_DISTRIBUIDO 
        	...
        	revocacionDto.setRegistroId(registro.getId());
    	...
    	model.addAttribute("revocacionDto", revocacionDto);
    ...
    @ResponseBody
    public JsonResponse enviarSir(
    		@PathVariable Long idRegistro, 
    		@RequestParam String oficinaSIRCodigo,
        ...
        try {
            ...
            synchronized (this) {
                ...
                if (permisoOrganismoUsuarioEjb.tienePermiso(usuarioEntidad.getId(), registroEntrada.getOficina().getOrganismoResponsable().getId(), RegwebConstantes.PERMISO_MODIFICACION_REGISTRO_ENTRADA, true)
                		...
                		&& !registroEntrada.getEstado().equals(RegwebConstantes.REGISTRO_RECHAZADO)
