    ...
    */
    public ParametrosRegistroModificado leer(ParametrosRegistroModificado param) {
		...
		try {
			...
			String sentenciaHql="SELECT * FROM BZMODIF WHERE FZJCENSA='S' AND FZJCAGCO=? AND FZJNUMEN=? AND FZJANOEN=? AND" +
    ...
    */
    public List recuperarRegistros(String oficina, String usuario) {
		...
		if (oficina.equals("00")) {
			...
			sentenciaHql="SELECT * FROM BZMODIF WHERE FZJCAGCO=? AND FZJFVISA=0 AND (FZJIEXTR=' ' OR FZJIREMI=' ' OR FZJIEXTR='' OR FZJIREMI='') AND FZJCENSA='S' ORDER BY " +
			"FZJCAGCO, FZJANOEN, FZJNUMEN, FZJFMODI, FZJHMODI";
		}
		try {
