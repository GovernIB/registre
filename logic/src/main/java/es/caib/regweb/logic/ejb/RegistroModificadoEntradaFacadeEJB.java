...
public abstract class RegistroModificadoEntradaFacadeEJB extends HibernateEJB {
    ...
    */
    public ParametrosRegistroModificado leer(ParametrosRegistroModificado param) {
		...
		try {
			...
			String sentenciaHql="SELECT * FROM BZMODIF WHERE FZJCENSA='E' AND FZJCAGCO=? AND FZJNUMEN=? AND FZJANOEN=? AND" +
			...
			if (rs.next()) {
	            ...
	            logLopdBZMODIF("SELECT", sessioEjb.getCallerPrincipal().getName().toUpperCase() 
						, fzafsis, horamili, 'E', param.getNumeroRegistro(), param.getAnoEntrada(), param.getOficina(), Integer.parseInt(param.getFechaModificacion()), Integer.parseInt(param.getHoraModificacion()) );
    ...
    */
		...
		try {
			...
			if (registrosAfectados>0 && (param.isHayVisadoExtracto() || param.isHayVisadoRemitente()) ) {
                ...
                try {
                    ...
                    Object [] params = {
                        "V", new Integer(param.getAnoEntrada()), new Integer(param.getNumeroRegistro()), new Integer(param.getOficina()), new Integer(param.getFechaDocumento()), rem, 
	...
	private boolean actualizarBZENTRA(ParametrosRegistroModificado param, Session session) throws HibernateException, ClassNotFoundException, Exception {
		...
		((param.isHayVisadoExtracto() && param.getIdiomaExtracto().equals("1")) ? ", FZACONEN=?" : "") +
		((param.isHayVisadoExtracto() && !param.getIdiomaExtracto().equals("1")) ? ", FZACONE2=?" : "") +
		//((param.isHayVisadoExtracto() && param.isHayVisadoRemitente()) ? ", " : "") +
		((param.isHayVisadoRemitente()) ? ", FZAREMIT=?, FZACENTI=?, FZANENTI=?" : "" ) +
		...
		try {
            ...
            int horamili=Integer.parseInt(hhmmss.format(fechaSystem)+Stringsss);  
            int fzafsis=Integer.parseInt(aaaammdd.format(fechaSystem));
            logLopdBZENTRA("UPDATE", param.getUsuarioModificacion()
    				, fzafsis, horamili, param.getNumeroRegistro(), param.getAnoEntrada(), param.getOficina());
		} catch (Exception e){
			log.error("Error: "+e.getMessage());
			generado=false;
			e.printStackTrace();
    ...
    */
    public List recuperarRegistros(String oficina, String usuario) {
		...
		if (oficina.equals("00")) {
			sentenciaHql="SELECT * FROM BZMODIF WHERE (FZJIEXTR=' ' OR FZJIREMI=' ' OR FZJIEXTR='' OR FZJIREMI='') AND FZJFVISA=0 AND FZJCAGCO " +
			...
			sentenciaHql="SELECT * FROM BZMODIF WHERE FZJCAGCO=? AND FZJFVISA=0 AND (FZJIEXTR=' ' OR FZJIREMI=' ' OR FZJIEXTR='' OR FZJIREMI='') AND FZJCENSA='E' ORDER BY " +
			"FZJCAGCO, FZJANOEN, FZJNUMEN, FZJFMODI, FZJHMODI";
		}
		try {
