...
/**
 * SessionBean per a manteniment de models d'ofici
 *
 * @ejb.bean
 *  name="logic/ModeloOficioFacade"
 *  jndi-name="es.caib.regweb.logic.ModeloOficioFacade"
 *  type="Stateless"
 *  view-type="remote"
 *  transaction-type="Container"
 *
 */
public abstract class ModeloOficioFacadeEJB extends HibernateEJB {
    ...
    */
    public ModeloOficio leer(String nomModel) throws ClassNotFoundException, Exception {

        Session session = getSession();
        String datosleidos = null;
        java.sql.Blob dades=null;
        ModeloOficio res = null;

        try {
            String sentenciaHql=" from ModeloOficio " +
                            "where nombre=? ";
            Query query=session.createQuery(sentenciaHql);
            query.setString(0,nomModel);
            java.util.List resultados = query.list();
            if (resultados!=null && resultados.size()>0 ) {
                res = (ModeloOficio) resultados.get(0);
            }
    ...
    */
    public void eliminar(String nomModel) throws ClassNotFoundException, Exception {
        ...
        try {
            String sentenciaHql= "delete from ModeloOficio  " +
                                 "where  ( nombre = ?)";
            Query query = session.createQuery(sentenciaHql);
            query.setString(0,nomModel);
            query.executeUpdate();
    ...
    */
    public boolean grabar(String nomModel, String conType, byte[] dades) throws ClassNotFoundException, Exception{
        ...
        try {
            ModeloOficio model = new ModeloOficio(nomModel, conType, dades);
