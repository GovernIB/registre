...
/**
 * SessionBean per a llistat de registres d'accés visats
 *
 * @ejb.bean
 *  name="logic/ListadoRegAccesosVisatFacade"
 *  jndi-name="es.caib.regweb.logic.ListadoRegAccesosVisatFacade"
 *  type="Stateless"
 *  view-type="remote"
...
//			Del tipus: 00:00:01:zzz
	...
	public Vector recuperar(ParametrosListadoAcceso parametros, String usuario, int sizePagina, int pagina) throws java.rmi.RemoteException {
		...
		try {
            // String sentenciaHql="select log.id.tipoAcceso, log.id.usuario, log.id.fecha, log.id.hora, log.id.numero, log.id.anyo, log.id.oficina, ofi.nombre, log.id.codigoEntradaSalida, log.id.horaModificacion " +
            //     " from LogModificacionLopd log, Oficina ofi " +
            //     " where log.id.oficina=ofi.codigo  and log.id.oficina>=? and log.id.oficina<=? " +
            ...
            // (!registreES.equals("") ? " and log.id.codigoEntradaSalida=?" : "" )+
			...
			q.addScalar("FZVTIPAC", Hibernate.STRING);
            q.addScalar("FZVCUSU" , Hibernate.STRING);			
            q.addScalar("FZVDATAC", Hibernate.INTEGER);
            q.addScalar("FZVHORAC", Hibernate.INTEGER);
            q.addScalar("FZVNUMEN", Hibernate.INTEGER);
            q.addScalar("FZVANOEN", Hibernate.INTEGER);
            q.addScalar("FZVCAGCO", Hibernate.INTEGER);
            q.addScalar("FZVCENSA", Hibernate.STRING);
            q.addScalar("FZVHMODI", Hibernate.INTEGER);
			...
			if (!registreES.equals(""))
				q.setString(contador++,registreES);
...
//				 Point to the last row in resultset.
			...
			while (rs.next()) {
				...
				registro.setTipusVisat(String.valueOf(rs.getString(7)));
				String fechaESModif=String.valueOf(rs.getInteger(2));
				try {
				...
				String hora=String.valueOf(rs.getInteger(8));
				String hhmm="";
