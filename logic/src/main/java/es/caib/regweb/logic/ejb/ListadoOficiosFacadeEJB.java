...
public abstract class ListadoOficiosFacadeEJB extends HibernateEJB {
    ...
    */
     ...
     * @ejb.permission unchecked="true"
     */
     public Vector recuperarRegistros(String usuario, String oficinaDesde, String oficinaHasta, String oficinaFisica, String pfechaDesde, String pfechaHasta)
 	        throws java.rmi.RemoteException, Exception {
 		...
 		usuario=usuario.toUpperCase();
 		Vector registrosVector = new Vector();
 		int fechaDesde = es.caib.regweb.logic.helper.Helper.convierteStringFechaAIntFecha(pfechaDesde);
 		int fechaHasta = es.caib.regweb.logic.helper.Helper.convierteStringFechaAIntFecha(pfechaHasta);
 		...
 		try {
 			String sentenciaSql= "SELECT FZSANOEN, FZSNUMEN, FAACAGCO, FAADAGCO, OFS_CODI, " +
 				...
 				" LEFT JOIN BZIDIOM ON FZSCIDI=FZMCIDI  " +
 				" LEFT JOIN BAGRUGE ON FZSCTAGG=FABCTAGG AND FZSCAGGE=FABCAGGE  " +
 				" LEFT JOIN BZAUTOR ON FZHCUSU=? AND FZHCAGCO=REM_OFOFI AND FZHCAUT=? " +
 				" WHERE REM_SALANY>2008 " +
 				"  AND REM_OFOFI >=" + oficinaDesde +
 				"  AND REM_OFOFI <=" + oficinaHasta +
 				"  AND REM_OFFEC >=" + fechaDesde +
 				"  AND REM_OFFEC <=" + fechaHasta +
 				(oficinaFisica==null || oficinaFisica.equals("")?"":" AND OFS_CODI = ? ") +
 				" ORDER BY FZSCAGCO, FZSANOEN, FZSNUMEN ";
 			q=session.createSQLQuery(sentenciaSql); //, ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.CONCUR_READ_ONLY);
             q.addScalar("FZSANOEN", Hibernate.INTEGER);
			...
			if (oficinaFisica!=null && !oficinaFisica.equals("")) {
 			...
 			while (rs.next()) {
 				...
 				try {
 					registro.setData(es.caib.regweb.logic.helper.Helper.convierteyyyymmddFechaAddmmyyyyFecha(fechaDocu));
 				} catch (Exception e) {
 					registro.setData(fechaDocu);
 				}
 				String fechaES=String.valueOf(rs.getInteger(7));
 				try {
 					registro.setFechaES(es.caib.regweb.logic.helper.Helper.convierteyyyymmddFechaAddmmyyyyFecha(fechaES));
 				} catch (Exception e) {
 					registro.setFechaES(fechaES);
 				}
